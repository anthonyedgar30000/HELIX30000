...
AUTHORITY_BY_ROLE = {
    "control_tower": "coordination_only",
...
def validate_registry(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, role_value in enumerate(roles):
        ...
        if not title.startswith(PREFIX_BY_ROLE[role_type]):
            raise RegistryValidationError(
                f"{field}.desired_chat_title must start with "
                f"{PREFIX_BY_ROLE[role_type]!r}"
            )
        if title in titles:
        ...
        owned_branch = _nullable_text(
            role.get("owned_branch"), f"{field}.owned_branch"
        )
        if owned_repository is not None and not REPOSITORY_RE.fullmatch(
            owned_repository
        ):
        ...
        if role_type == "write_owner":
            if owned_repository is None or owned_branch is None:
                raise RegistryValidationError(
                    f"{field} write_owner requires owned repository and branch"
                )
            if not (allowed & WRITE_ACTIONS):
                raise RegistryValidationError(
                    f"{field} write_owner requires bounded repository write action"
                )
            writer_key = (owned_repository, owned_branch)
            if writer_key in confirmed_writers:
                raise RegistryValidationError(
                    f"multiple confirmed writers for "
                    f"{owned_repository}:{owned_branch}"
                )
            confirmed_writers.add(writer_key)
        else:
            if owned_repository is not None or owned_branch is not None:
                raise RegistryValidationError(
                    f"{field} non-write-owner role cannot own a repository or branch"
                )
            if allowed & WRITE_ACTIONS:
                raise RegistryValidationError(
                    f"{field} non-write-owner role cannot allow repository writes"
                )
        ...
        target_branch = _nullable_text(
            target.get("branch"), f"{field}.target.branch"
        )
        target_pr = target.get("pull_request")
        head_sha = _nullable_text(
            target.get("head_sha"), f"{field}.target.head_sha"
        )
        if target_repository is not None and not REPOSITORY_RE.fullmatch(
            target_repository
        ):
            raise RegistryValidationError(
                f"{field}.target.repository is not owner/repo"
            )
        if target_pr is not None:
            if (
                not isinstance(target_pr, int)
                or isinstance(target_pr, bool)
                or target_pr < 1
            ):
                raise RegistryValidationError(
                    f"{field}.target.pull_request is invalid"
                )
        ...
        if head_sha is not None and not SHA_RE.fullmatch(head_sha):
            raise RegistryValidationError(
                f"{field}.target.head_sha is not a full SHA"
            )

        if role_type == "write_owner":
            if target_repository != owned_repository or target_branch != owned_branch:
                raise RegistryValidationError(
                    f"{field} target must match the write-owner repository and branch"
                )
        ...
        _text(
            ...
            f"{field}.evidence.evidence_strength",
        )
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"conversation-role registry validation failed: {exc}",
            file=sys.stderr,
        )
        return 1
